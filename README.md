...
│   ├── portfolio-health-check/         # Portfolio diagnostics    ← NEW
│   ├── suitability-report-generator/   # Investment documentation ← NEW
│   ├── financial-statement-analyzer/   # Financial deep dive      ← NEW
│   ├── event-driven-detector/          # Special situations       ← NEW
│   ├── quant-factor-screener/          # Multi-factor screening   ← NEW
│   └── esg-screener/                   # ESG analysis             ← NEW
...
│   ├── portfolio-health-check/         # 组合健康诊断             ← NEW
│   ├── suitability-report-generator/   # 投资适当性报告           ← NEW
│   ├── financial-statement-analyzer/   # 财务报表深度分析         ← NEW
│   ├── event-driven-detector/          # 事件驱动机会             ← NEW
│   ├── quant-factor-screener/          # 量化因子筛选             ← NEW
│   └── esg-screener/                   # ESG筛选                  ← NEW
...
- *"Review my portfolio for hidden risks and concentration issues"* ← NEW
- *"Generate a suitability report for this portfolio recommendation"* ← NEW
- *"Do a deep dive into Apple's financial statements"* ← NEW
- *"What merger arbitrage opportunities are available right now?"* ← NEW
- *"Screen stocks using a multi-factor model with value and quality"* ← NEW
- *"Find the best ESG-rated companies in the S&P 500"* ← NEW
...
- *"推荐几只市值小但增长快的专精特新公司"*
- *"帮我诊断一下我的持仓有什么风险"* ← NEW
- *"为这个投资建议生成一份适当性报告"* ← NEW
- *"深度分析一下贵州茅台的财务报表"* ← NEW
- *"最近有哪些A股并购重组机会？"* ← NEW
- *"用多因子模型帮我筛选A股"* ← NEW
- *"帮我找ESG评分最高的沪深300成分股"* ← NEW
