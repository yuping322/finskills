...
- `views_runner.py` 输出统一为 JSON：`{meta, data, warnings, errors}`。
...
| --- | --- | --- | --- | --- |
| sse_summary | stock_sse_summary | 上海证券交易所-股票数据总貌 | - | (columns as-is) |
| szse_summary | stock_szse_summary | 深圳证券交易所-市场总貌-证券类别统计 | required: date | 证券类别, 数量, 成交金额, 总市值, 流通市值 |
