...
2. **风险指标**：
   ...
   - `组合VaR(95%) = percentile(组合日收益率, 5%)` (%)

3. **持仓集中度指标**：
   - `持仓集中度HHI = Σ(持仓权重^2)` (0-1，越大越集中)
   - `前十大持仓占比 = Σ(前十大持仓权重)` (%)
   - `单一持仓最大权重 = max(持仓权重)` (%)
   - `行业集中度 = Σ(行业权重^2)` (0-1)
   - `有效持仓数 = 1 / HHI` (只，等效分散持仓数)

4. **风险预警指标**：
   - `解禁风险股数 = count(解禁风险 = 高)` (只)
   - `质押风险股数 = count(质押风险 = 高)` (只)
   - `ST风险股数 = count(ST风险 = 高)` (只)
   - `商誉风险股数 = count(商誉风险 = 高)` (只)
   - `综合风险评分 = 100 - Σ(风险权重 × 风险得分)` (0-100，越低风险越高)
...
- **0.3持仓集中度HHI**：HHI ≥ 0.3属于高集中度（等效持仓数 ≤ 3.3只）
- **20%单一持仓**：单一持仓 ≥ 20%属于过度集中（风险过高）
- **20%最大回撤**：最大回撤 ≥ 20%属于风险过大（需要风控）
- **0.5夏普比率**：夏普比率 < 0.5属于风险调整后收益差（策略无效）
- **2只风险股**：风险股数 ≥ 2属于多重风险（需要降低仓位）
- **40%前十大持仓占比**：前十大持仓占比 ≥ 40%属于集中持仓
- **30%波动率**：组合波动率 ≥ 30%属于高波动（风险较高）
- **10%回撤预警**：回撤 ≥ 10%触发预警，≥ 20%触发强制风控
